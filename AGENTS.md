...
* Database migration：時間戳加清楚的 snake\_case 描述  
* Test：描述使用者行為與可觀察結果
...
DATABASE\_ERROR  
AI\_TIMEOUT  
AI\_RESPONSE\_INVALID  
AUDIO\_UPLOAD\_FAILED  
CONTENT\_NOT\_PUBLISHED
...
* 使用者 A 不可刪除使用者 B 的資料。  
* learner 不可執行管理操作。  
* content\_editor、reviewer、admin 權限符合 contract。  
...
AI\_EVALUATION\_FAKE\_MODE=true
