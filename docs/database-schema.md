...
| ai_feedback            | id, user_id, attempt_id, feature, target_type, target_id, schema_version, prompt_id, prompt_version, model, feedback_json, requires_human_review, cache_key, idempotency_key, cached_from_id                  | unique(user_id,idempotency_key), index(user_id,created_at), index(user_id,feature,cache_key) | self read; service-only transaction write           |
